...
const handleNextStep = (params = {}) => {
  ...
  // 如果有Custom rounds数，通过 query 参数传递
  if (params.maxRounds) {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog(`Simulation close error: ${closeErr.message}`)
        // 如果优雅CloseError，尝试强制停止
        await forceStopSimulation()
      }
    } else {
      // 环境未运行，但可能进程还在，检查模拟State
