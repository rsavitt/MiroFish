<template>
  ...
  >
    <!-- Background: tech grid (only shown when projects exist) -->
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        <!-- 卡片头部：simulation_id 和 功能可用State -->
        ...
        <!-- 卡片标题（使用Simulation Requirements的前20字作为标题） -->
        <h3 class="card-title">{{ getSimulationTitle(project.simulation_requirement) }}</h3>

        <!-- 卡片描述（Simulation Requirements完整展示） -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 (Simulation) and Step 5 (Interaction) require a live session — no replay available</span>
            </div>
          </div>
        </div>
      </Transition>
...
let pendingState = null  // 记录待执行的目标State
...
// 从Simulation Requirements生成标题（取前20字）
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // 更新待执行的目标State（None论是否在动画中都要记录最新的目标State）
        ...
        // 如果正在动画中，只记录State，等动画结束后处理
        if (isAnimating) return
        
        // 如果目标State与当前State相同，不需要处理
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // 使用防抖延迟State切换，防止快速闪烁
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // 动画Done后解除锁定，并检查是否有待处理的State变化
          setTimeout(() => {
            isAnimating = false
            
            // 动画结束后，检查是否有新的待执行State
...
onMounted(async () => {
  // 确保 DOM 渲染Done后再Load数据
  await nextTick()
  await loadHistory()
  
  // Pending DOM 渲染后Initializing观察器
  setTimeout(() => {
    initObserver()
  }, 100)
})

// 如果使用 keep-alive，在组件激活时重新Load数据
onActivated(() => {
  loadHistory()
})
...
.status-icon:nth-child(2).available { color: #F59E0B; } /* Env Setup - 橙色 */
