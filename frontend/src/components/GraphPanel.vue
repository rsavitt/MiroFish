    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content still processing, try refreshing later</span>
...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // 自环边 - 每 agents Nodes只添加一 items合并的自环
  ...
  // Simulation - 根据边数量动态调整 Nodes间距
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // 根据这对 Nodes之间的边数量动态调整距离
      // 基础距离 150，每多一 items边增加 40
  ...
  const getLinkPath = (d) => {
    ...
    // 检测自环
    if (d.isSelfLoop) {
      // 自环：绘制一 agents圆弧从 Nodes出发再Back
      const loopRadius = 30
      // 从 NodesRight side出发，绕一圈回来
